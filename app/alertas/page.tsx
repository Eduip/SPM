...
import AlertasDashboard, {
  AlertHistoryItem,
  AlertSeverity,
  AlertStatus,
  SystemAlert,
} from '../../components/alertas/AlertasDashboard'
...
export default async function AlertasPage() {
  ...
  if (proyectosError) {
    return (
      <AppShell title="Gestión de Alertas" currentModule="alertas">
        <ErrorCard message={`Error al cargar proyectos: ${proyectosError.message}`} />
      </AppShell>
    )
  }
  ...
  if (loadError) {
    return (
      <AppShell title="Gestión de Alertas" currentModule="alertas">
        <ErrorCard message={`Error al cargar datos de alertas: ${loadError.message}`} />
      </AppShell>
    )
  }
  ...
  const alertas = buildSystemAlerts({
    proyectos,
  ...
  return (
    <AppShell title="Gestión de Alertas" currentModule="alertas">
      <AlertasDashboard alerts={alertas} />
    </AppShell>
  )
}
...
function buildSystemAlerts({
  ...
  const activeRules = reglas.map(toRule).filter((rule): rule is ReturnType<typeof toRule> & { config: RuleConfig } => Boolean(rule?.config))
    ...
    for (const rule of activeRules) {
      ...
      if (rule.config.field === 'dias_sin_rendicion') {
        for (const proyecto of proyectos) {
          ...
          if (!matchesRule(days, rule.config) || (rendicionesProyecto.length > 0 && !hasOpenRendition)) {
            continue
          }
      ...
      if (rule.config.field === 'porcentaje_presupuesto_ejecutado') {
        for (const proyecto of proyectos) {
          ...
          const presupuesto = presupuestoPorProyecto.get(proyecto.id) ?? Number(proyecto.monto_estimado ?? 0)
